...
export default function App() {
  ...
  const generateImage = async () => {
    ...
    try {
      ...
      let prompt = "";
      if (angle === 'default') {
        prompt = "ARCHITECTURAL TRANSFORMATION: 1. STRICTLY PRESERVE the exact number, boundaries, proportions, and shapes of the original windows/doors from image 1. DO NOT merge multiple separate windows into one or alter their original layout. 2. Replace all windows/doors with the EXACT system window model from the product image (image 2), matching the EXACT frame color, edge thickness, metallic finish, and hardware details of image 2. 3. CRITICAL: Replicate the EXACT internal mullion patterns, decorative grilles, and lattice designs from image 2. 4. If image 1 is interior, beautify walls/furniture; if exterior, upgrade facade to luxury minimalism with smooth, high-end seamless textures, STRICTLY NO TILES. 5. Final output must match the intricate window patterns and exact colors of image 2 perfectly.";
      } else if (angle === 'interior') {
        prompt = "MODERN LIGHT LUXURY INTERIOR:  1. STRICTLY PRESERVE the exact number, boundaries, proportions, and structural framework of the original windows from image 1. DO NOT merge multiple windows into a single large window or change their shapes. 2. Apply the EXACT system window design, EXACT frame color, edge thickness, metallic finish, and internal decorative patterns from the product image (image 2). CRITICAL: Replicate the EXACT grille type—whether vertical-only, horizontal-only, cross-grid, or arched radial patterns—along with EXACT glass transparency (clear, frosted, or textured) from image 2. 3. Transform the room into a modern light luxury style with elegant high-end furniture (sofa, coffee table), marble flooring, and sophisticated ambient lighting. 4. Maintain professional architectural photography quality.";
      } else if (angle === 'high') {
        prompt = "REALISTIC DRONE AERIAL PHOTOGRAPHY: 1. STRICTLY PRESERVE the exact number, boundaries, and shapes of the original windows/doors from the villa image (image 1). DO NOT merge or alter their structural layout. 2. Replace all windows/doors with the EXACT system window model, frame color, and patterns from image 2. 3. Ensure the final result maintains the perspective of image 1 but as a finished luxury villa with realistic landscaping, NO SWIMMING POOLS. 4. The window details must match image 2 with 100% fidelity. 5. Aim for a natural, authentic drone shot feel.";
